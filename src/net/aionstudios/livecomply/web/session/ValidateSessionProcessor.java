package net.aionstudios.livecomply.web.session;
...
public class ValidateSessionProcessor extends Processor {
	...
	public void compute(HttpExchange he, RequestVariables vars, Map<String, Object> pageVariables) {
		if(vars.getCookieManager().getRequestCookies().containsKey("sessionID") && vars.getCookieManager().getRequestCookie("sessionID").isEmpty()) {
			...
			sessionCookie.setDomain("wwa.aionstudios.net");
	...
	private String findSessionQuery = "SELECT * FROM `livecomply`.`web_sessions` WHERE `sessionID` = ?;";
	private String insertSessionQuery = "INSERT INTO `livecomply`.`web_sessions` (`sessionID`) VALUES (?);";
	private String updateSessionAge = "UPDATE `livecomply`.`web_sessions` SET `last` = CURRENT_TIMESTAMP WHERE `sessionID` = ?;";
